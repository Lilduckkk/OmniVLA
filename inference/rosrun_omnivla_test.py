...
class Inference:
    ...
    def run_omnivla(self):
        ...
        self.publish_waypoint_pose(chosen_waypoint, self.robot_pose)
        # PD controller
        EPS = 1e-8
        DT = 1 / 3
    ...
    def publish_waypoint_pose(self, waypoint):
        """
        发布路径点的位姿信息
        waypoint: [dx, dy, hx, hy] 在机器人局部坐标系下
        """
        pose_msg = PoseStamped()
        pose_msg.header = Header()
        pose_msg.header.stamp = rospy.Time.now()
        pose_msg.header.frame_id = "base_link"  # 相对于机器人基座标系
        
        # 位置 (x, y, z)
        pose_msg.pose.position = Point(x=waypoint[0], y=waypoint[1], z=0.0)
        
        # 方向 - 从 hx, hy 计算偏航角
        yaw = math.atan2(waypoint[3], waypoint[2])  # atan2(hy, hx)
        ...
        # 打印调试信息
        rospy.loginfo(f"Published waypoint pose: position=({waypoint[0]:.3f}, {waypoint[1]:.3f}, 0.0), yaw={yaw:.3f} rad")


    def publish_waypoint_pose(self, waypoint, robot_pose=None):
        """
        发布路径点的位姿信息
        waypoint: [dx, dy, hx, hy] 在机器人局部坐标系下
        robot_pose: 机器人的全局位姿 (可选)
        """
        pose_msg = PoseStamped()
        pose_msg.header = Header()
        pose_msg.header.stamp = rospy.Time.now()
        ...
        if robot_pose is not None:
            # 发布全局坐标系下的位姿
            pose_msg.header.frame_id = "world"  # 或您使用的全局坐标系
            
            # 将局部坐标转换到全局坐标系
            # 这里需要机器人的当前位姿信息
            global_x = robot_pose.position.x + waypoint[0] * math.cos(robot_pose.orientation.z) - waypoint[1] * math.sin(robot_pose.orientation.z)
            global_y = robot_pose.position.y + waypoint[0] * math.sin(robot_pose.orientation.z) + waypoint[1] * math.cos(robot_pose.orientation.z)
            global_yaw = robot_pose.orientation.z + math.atan2(waypoint[3], waypoint[2])
            
            pose_msg.pose.position = Point(x=global_x, y=global_y, z=0.0)
            quat = quaternion_from_euler(0, 0, global_yaw)
            pose_msg.pose.orientation = Quaternion(x=quat[0], y=quat[1], z=quat[2], w=quat[3])
            self.pose_pub.publish(pose_msg)
        else:
            # 发布局部坐标系下的位姿
            ...
            # pose_msg.pose.position = Point(x=waypoint[0], y=waypoint[1], z=0.0)
            # yaw = math.atan2(waypoint[3], waypoint[2])
            # quat = quaternion_from_euler(0, 0, yaw)
            print('goal is none')
    ...
    def save_robot_behavior(self, cur_img, goal_img, goal_pose, waypoints,
        ...
        save_path = os.path.join(self.datastore_path_image, f"1_ex.jpg")
...
class OmniVLANode:
    def __init__(self, inference):
        ...
        rospy.Subscriber("/camera/color/image_raw", ROSImage, self.image_callback)
        rospy.Subscriber('/odom/base', Odometry, self.odom_callback)
        # 新增：定时器，每隔1秒调用一次 timer_callback
        self.timer = rospy.Timer(rospy.Duration(0.05), self.timer_callback)
        self.linear_vel = 0.0
        self.angular_vel = 0.0
        self.robot_pose = None
        self.inference.robot_pose = None
    def image_callback(self, msg):
        try:
            cv_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
            pil_image = Image.fromarray(cv_image[:, :, ::-1].copy())
            current_image_path = "./inference/current_img.jpg"
            pil_image.save(current_image_path)
            self.linear_vel, self.angular_vel = self.inference.run_omnivla()
            # twist = Twist()
            # twist.linear.x = linear_vel
            # twist.angular.z = angular_vel
            # self.cmd_vel_pub.publish(twist)
        except Exception as exc:
            rospy.logerr(f"image_callback failed: {exc}")

    def odom_callback(self, msg):
        try:
            self.robot_pose = msg.pose.pose
            self.inference.robot_pose = msg.pose.pose
            # twist = Twist()
            # twist.linear.x = linear_vel
            # twist.angular.z = angular_vel
            # self.cmd_vel_pub.publish(twist)
    ...
    def timer_callback(self, event):
        try:
            twist = Twist()
            twist.linear.x = self.linear_vel / 2
            twist.angular.z = self.angular_vel / 2
            # self.cmd_vel_pub.publish(twist)
        except Exception as exc:
            rospy.logerr(f"timer_callback failed: {exc}")

class OmniVLANode_test:
    def __init__(self):
        self.bridge = CvBridge()
        self.cmd_vel_pub = rospy.Publisher("/cmd_vel", Twist, queue_size=10)
        ...
        self.linear_vel = 0.0
        self.angular_vel = 0.0
        print("OmniVLANode_test initialized.")
    ...
    def timer_callback(self, event):
        try:
            ...
            self.cmd_vel_pub.publish(twist)
            print("Published cmd_vel:", twist.linear.x, twist.angular.z)
        except Exception as exc:
            rospy.logerr(f"timer_callback failed: {exc}")
# ===============================================================
# Main Entry
...
if __name__ == "__main__":
    ...
    # lan_inst_prompt = "turn right and move toward the blue door"
    # lan_inst_prompt = "move toward the person in a brown top"
    # lan_inst_prompt = "pass through the chair from the right side"
    # lan_inst_prompt = "move toward the black chair"
    # lan_inst_prompt = "go straight along the white wall"
    # lan_inst_prompt = "move to the right of the chair"
    # lan_inst_prompt = "move to the person from the right side of the chair"
    # lan_inst_prompt = "move along the left wall"
    # lan_inst_prompt = "move toward the person along the left wall"
    lan_inst_prompt = "move toward the white office cabinet"
    # lan_inst_prompt = "move toward the white table"
    ...
    rospy.init_node("omnivla_node", anonymous=True)
    OmniVLANode(inference)
    # OmniVLANode_test()
    rospy.spin()
