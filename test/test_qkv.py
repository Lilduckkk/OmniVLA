...
class Inference:
    def __init__(self, save_dir, lan_inst_prompt, goal_utm, goal_compass, goal_image_PIL, 
                 action_tokenizer, processor, test_id, qkv_save_dir):  # 新增参数
        self.tick_rate = 3
        ...
        self.test_id = test_id  # 当前测试编号
        self.qkv_save_dir = qkv_save_dir  # QKV保存目录
    # ----------------------------  # 分隔线
    ...
    def run_omnivla(self):  # 推理主流程
        ...
        current_image_path = f"./inference/current_img_{self.test_id}.jpg"  # 不同test_id对应不同图像
        current_image_PIL = Image.open(current_image_path).convert("RGB")
        print(f"第{self.test_id}次测试使用的当前图像: {current_image_path}")
    ...
    def data_transformer_omnivla(self, current_image_PIL, lan_inst, goal_image_PIL, goal_pose_loc_norm,
        ...
        # 为当前测试生成唯一随机动作（用test_id做种子确保可复现）
        # np.random.seed(self.test_id)  # 不同test_id生成不同动作
        # actions = np.random.rand(8, 4)  # 8步动作，每步4维
        actions = np.random.uniform(-1.0, 1.0, (8, 4))  # 固定动作

        goal_pose_cos_sin = goal_pose_loc_norm

        # 保存当前测试的动作序列
        action_save_path = os.path.join(self.qkv_save_dir, f"action_test_{self.test_id}.npy")
        np.save(action_save_path, actions)
        print(f"已保存第{self.test_id}次测试的动作序列: {action_save_path}")
    ...
    def run_forward_pass(self, vla, action_head, noisy_action_projector, pose_projector,
        ...
        text_queries, text_keys, text_values = None, None, None
        ...
        if last_layer is not None and hasattr(last_layer, 'self_attn'):
            ...
            if hasattr(self_attn, 'q_proj'):
                ...
                # 保存到本地
                np.save(
                    os.path.join(self.qkv_save_dir, f"queries_test_{self.test_id}.npy"),
                    text_queries.detach().to(torch.float32).cpu().numpy()  # 转换为float32
                )
                np.save(
                    os.path.join(self.qkv_save_dir, f"keys_test_{self.test_id}.npy"),
                    text_keys.detach().to(torch.float32).cpu().numpy()
                )
                np.save(
                    os.path.join(self.qkv_save_dir, f"values_test_{self.test_id}.npy"),
                    text_values.detach().to(torch.float32).cpu().numpy()
                )
                print(f"已保存第{self.test_id}次测试的QKV: {self.qkv_save_dir}")
...
#     # 新增：测试配置
#     # ==============================================
#     test_times = 5  # 测试5组不同动作
#     qkv_save_dir = "./test/qkv_results"
#     os.makedirs(qkv_save_dir, exist_ok=True)  # 创建保存目录

#     # 循环测试不同随机动作
#     for test_id in range(test_times):
#         print(f"\n===== 第 {test_id+1}/{test_times} 次测试 =====")
#         # 实例化推理类（传入当前测试ID和QKV保存目录）
#         inference = Inference(
#             save_dir="./inference",
...
#             action_tokenizer=action_tokenizer,
#             processor=processor,
#             test_id=test_id,  # 传入测试ID
#             qkv_save_dir=qkv_save_dir  # 传入QKV保存目录
#         )
#         inference.run()  # 执行推理
...
#     print("\n===== QKV差异对比结果 =====")
#     # 以第0次测试为基准
#     q0 = np.load(os.path.join(qkv_save_dir, "queries_test_0.npy"))
#     for i in range(1, test_times):
#         qi = np.load(os.path.join(qkv_save_dir, f"queries_test_{i}.npy"))
#         # 计算平均绝对差异（排除数值精度误差）
#         diff = np.mean(np.abs(q0 - qi))
#         print(f"第0次与第{i}次测试的Queries平均差异: {diff:.6f}")
#         if diff > 1e-5:
#             print(f"→ 差异显著：不同动作导致QKV变化")
#         else:
#             print(f"→ 差异微小：可能存在逻辑问题")
...
if __name__ == "__main__":
    ...
    satellite = False
    image_goal = False
    lan_prompt = True  # 语言指令固定

    # 目标定义（固定）
    lan_inst_prompt = "move toward white office cabinet"  # 固定语言指令
    goal_lat, goal_lon, goal_compass = 37.8738930785863, -122.26746181032362, 0.0  # 固定目标位姿
    goal_utm = utm.from_latlon(goal_lat, goal_lon)
    goal_compass = -float(goal_compass) / 180.0 * math.pi
    goal_image_PIL = Image.open("./inference/goal_img.jpg").convert("RGB")  # 固定目标图像
    ...
    # 测试配置（验证5张不同图像）
    test_times = 5
    qkv_save_dir = "./inference/qkv_image_test"  # 新目录保存图像测试结果
    os.makedirs(qkv_save_dir, exist_ok=True)

    # 循环测试（仅当前图像不同，其他输入完全一致）
    for test_id in range(test_times):
        print(f"\n===== 第 {test_id+1}/{test_times} 次测试（不同当前图像） =====")
        inference = Inference(
            save_dir="./inference",
            lan_inst_prompt=lan_inst_prompt,
            goal_utm=goal_utm,
    ...
    for i in range(1, test_times):
        qi = np.load(os.path.join(qkv_save_dir, f"queries_test_{i}.npy"))
        diff = np.mean(np.abs(q0 - qi))
        print(f"第0张与第{i}张图像的Queries平均差异: {diff:.6f}")
        if diff > 1e-5:
            print(f"→ 差异显著：不同当前图像导致QKV变化")
        else:
            print(f"→ 差异微小：当前图像未影响QKV")