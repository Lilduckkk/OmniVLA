...
class LanguageOnlyInference:
    ...
    def __init__(self, save_dir, lan_inst_prompt, processor, test_id, qkv_save_dir, action_seed=None):
        """
        参数:
            save_dir: 保存结果的根目录
...
if __name__ == "__main__":
    ...
    k0 = np.load(os.path.join(qkv_save_dir, "keys_test_0.npy"))
    v0 = np.load(os.path.join(qkv_save_dir, "values_test_0.npy"))
    ...
    # ===========================
    # 测试2：不同语言指令的QKV差异（修正版）
    # ===========================
    print(f"\n{'='*60}")
    print(f"测试2：不同语言指令的QKV差异")
    print(f"{'='*60}")
    
    qkv_save_dir_diff = "./test/qkv_language_diff"
    os.makedirs(qkv_save_dir_diff, exist_ok=True)
    
    different_prompts = [
        "move toward white office cabinet",
        "go to the red chair",
        "navigate to the door"
    ]
    
    for test_id, prompt in enumerate(different_prompts):
        print(f"\n{'='*60}")
        print(f"第 {test_id+1}/{len(different_prompts)} 次测试: {prompt}")
        print(f"{'='*60}")
        
        inference = LanguageOnlyInference(
            save_dir="./inference",
            lan_inst_prompt=prompt,
            processor=processor,
            test_id=test_id,
            qkv_save_dir=qkv_save_dir_diff,
            action_seed=GLOBAL_SEED
        )
        inference.run()
    
    # 对比不同语言指令的QKV差异（修正：处理不同长度）
    print("\n" + "="*60)
    print("QKV差异对比结果（不同语言指令）")
    print("="*60)
    
    q0_diff = np.load(os.path.join(qkv_save_dir_diff, "queries_test_0.npy"))
    
    for i in range(1, len(different_prompts)):
        qi_diff = np.load(os.path.join(qkv_save_dir_diff, f"queries_test_{i}.npy"))
        
        # 获取两个序列的形状
        seq_len_0 = q0_diff.shape[1]
        seq_len_i = qi_diff.shape[1]
        
        print(f"\n指令0 vs 指令{i}:")
        print(f"  '{different_prompts[0]}' (长度: {seq_len_0})")
        print(f"  '{different_prompts[i]}' (长度: {seq_len_i})")
        
        # 方法1：截取公共长度部分对比
        min_len = min(seq_len_0, seq_len_i)
        q0_trimmed = q0_diff[:, :min_len, :]
        qi_trimmed = qi_diff[:, :min_len, :]
        diff_trimmed = np.mean(np.abs(q0_trimmed - qi_trimmed))
        print(f"  Queries差异 (前{min_len}个token): {diff_trimmed:.6f}")
        
        # 方法2：对比统计特征（均值/标准差）
        q0_mean = np.mean(q0_diff)
        qi_mean = np.mean(qi_diff)
        q0_std = np.std(q0_diff)
        qi_std = np.std(qi_diff)
        
        mean_diff = np.abs(q0_mean - qi_mean)
        std_diff = np.abs(q0_std - qi_std)
        
        print(f"  均值差异: {mean_diff:.6f}")
        print(f"  标准差差异: {std_diff:.6f}")
        
        # 方法3：余弦相似度（展平后）
        q0_flat = q0_diff.flatten()
        qi_flat = qi_diff.flatten()
        
        # 如果长度不同，填充较短的
        if len(q0_flat) < len(qi_flat):
            q0_flat = np.pad(q0_flat, (0, len(qi_flat) - len(q0_flat)), mode='constant')
        elif len(qi_flat) < len(q0_flat):
            qi_flat = np.pad(qi_flat, (0, len(q0_flat) - len(qi_flat)), mode='constant')
        
        cosine_sim = np.dot(q0_flat, qi_flat) / (np.linalg.norm(q0_flat) * np.linalg.norm(qi_flat))
        print(f"  余弦相似度: {cosine_sim:.6f} (1.0=完全相同, 0.0=正交)")
        
        # 综合判断
        if diff_trimmed > 0.1 or mean_diff > 0.01 or cosine_sim < 0.99:
            print(f"  → 结论: 差异显著 ✅ (不同语言指令产生不同QKV)")
        else:
            print(f"  → 结论: 差异微小 ❌ (可能存在问题)")
