...
class OmniVLAConfig:
    ...
    wandb_entity: str = "lilduck_sustech"            # Name of WandB entity
...
def run_forward_pass(
    ...
    ground_truth_actions = batch["original_normalized_trajectory"].to(device_id).to(torch.bfloat16)
    ...
    if True:
        ...
        action_ref = ground_truth_actions # action_ref为裁剪后的八个点, ground_truth_actions为固定的20个点
        ...
        loss = 0.1*torch.nn.MSELoss()(ground_truth_actions[:,-1,0:2], predicted_actions[:,-1,0:2][lan_bool]) + 0.1*torch.nn.MSELoss()(predicted_actions[:,0:-1], predicted_actions[:,1:]) + 1*cost_traj         
        # print(f"loss:{loss} ")   
        L2_action = torch.nn.MSELoss()(action_ref, predicted_actions)
        # L2_obj = torch.nn.MSELoss()(obj_pose_norm[lan_bool], predicted_actions[:,-1,0:2][lan_bool]) # obj_pose_norm是相对位姿，这里不好直接用
        L2_obj = torch.nn.MSELoss()(ground_truth_actions[:,-1,0:2][lan_bool], predicted_actions[:,-1,0:2][lan_bool])
...
def run_evaluation(
    ...
    """
    Run evaluation loop on the test dataset with Visualization.
    """
    vla.eval()
    action_head.eval()
    pose_projector.eval()
    ...
    # 进度条
    iterator = tqdm.tqdm(test_loader, desc=f"Evaluating Epoch {epoch}", disable=not distributed_state.is_main_process)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(iterator):
            ...
            # 准备数据
            modality_id = merged_batch["goal_mask_select"]
            # ground_truth_actions = merged_batch["actions"].to(device_id).to(torch.bfloat16)
            # 替换ground_truth_actions为我们新定义的
            ground_truth_actions = merged_batch["original_normalized_trajectory"].to(device_id).to(torch.bfloat16)
            obj_pose_norm = merged_batch["obj_pose_norm"].to(dtype=torch.bfloat16).to(device_id)
            
            # 2. Forward Pass (显式执行以获取 predicted_actions)
            #    注意：这里复制了 run_forward_pass 的推理部分，为了拿到 predicted_actions 做绘图
            with torch.autocast("cuda", dtype=torch.bfloat16):
                output = vla(
                    input_ids=merged_batch["input_ids"].to(device_id),
                    attention_mask=merged_batch["attention_mask"].to(device_id),
                    attention_mask_label=merged_batch["attention_mask_label"].to(device_id),                  
                    pixel_values=merged_batch["pixel_values"].to(torch.bfloat16).to(device_id),
                    modality_id=modality_id.to(torch.bfloat16).to(device_id),
                    labels=merged_batch["labels"],
                    output_hidden_states=True,               
                    proprio=merged_batch["goal_pose"].to(torch.bfloat16).to(device_id),
                    proprio_projector=pose_projector,
                    use_film=False,
                )
            ...
            # 计算 Action Mask
            ground_truth_token_ids = merged_batch["labels"][:, 1:].to(device_id)
            current_action_mask = get_current_action_mask(ground_truth_token_ids)
            next_actions_mask = get_next_actions_mask(ground_truth_token_ids)
            
            batch_size_curr = merged_batch["input_ids"].shape[0]
            actions_hidden_states = (
                text_hidden_states[current_action_mask | next_actions_mask]
                .reshape(batch_size_curr, NUM_ACTIONS_CHUNK * ACTION_DIM, -1)
                .to(torch.bfloat16)
            )
            ...
            # 3. 计算 Loss / Metrics (简单起见，我们只计算关键的 MSE Loss)
            #    如果需要完整的 loss 列表，可以复制 run_forward_pass 里的 metrics 计算逻辑
            action_ref = ground_truth_actions
            action_ref = merged_batch["actions"].to(device_id).to(torch.bfloat16)
            lan_bool = (merged_batch["goal_mask_select"] == 7)|(merged_batch["goal_mask_select"] == 8) #object loss is only for the LeLaN dataset
            traj_cost = TrajCost(merged_batch)
            cost_traj = traj_cost.CostofTraj( predicted_actions)
            cost_mask = traj_cost.CostofSegMask( predicted_actions)            
            # loss = torch.nn.MSELoss()(action_ref, predicted_actions)
            # loss = 1.0*torch.nn.MSELoss()(action_ref, predicted_actions) + 0.1*torch.nn.MSELoss()(obj_pose_norm[lan_bool], predicted_actions[:,-1,0:2][lan_bool]) + 0.1*torch.nn.MSELoss()(predicted_actions[:,0:-1], predicted_actions[:,1:]) 
            # loss = 1*torch.nn.MSELoss()(obj_pose_norm[lan_bool], predicted_actions[:,-1,0:2][lan_bool]) + 1*torch.nn.MSELoss()(predicted_actions[:,0:-1], predicted_actions[:,1:]) 
            loss = 0.1*torch.nn.MSELoss()(ground_truth_actions[:,-1,0:2], predicted_actions[:,-1,0:2][lan_bool]) + 0.1*torch.nn.MSELoss()(predicted_actions[:,0:-1], predicted_actions[:,1:]) + 1*cost_traj           
            # print(f"loss:{loss} ")   
            ...
            metrics = {
                ...
                # "L2_action_value": loss.item(), # 近似等于 Loss
                "L2_obj_value": L2_obj.item(),
            ...
            # 4. 可视化 (Visualization)
            #    只在主进程，且只在测试集的第一个 Batch 进行可视化，避免生成太多图片
            if distributed_state.is_main_process:
                # print("Generating visualization for validation set...")
                
                # 确定可视化的数量 (防止 batch size 太小)
                # vis_count = min(len(merged_batch["input_ids"]), 4) 
                vis_count = len(merged_batch["input_ids"])
                # vis_count = 1
                # print(f"Test Visualizing {vis_count} samples...")
                # 调用你现有的 visualize_train_new 函数
                visualize_train_new(
                    batch_current_PIL=merged_batch["img_PIL"], # 注意：merge_batches_padding 需要确保能处理 list
                    batch_goal_PIL=merged_batch["gimg_PIL"],              
                    goal_pos_lan=obj_pose_norm.detach().cpu(),   
                    goal_pos=merged_batch["goal_pose"].detach().cpu(),
                    traj_raw=ground_truth_actions.detach().cpu(), # 真实轨迹
                    est_traj=predicted_actions.detach().cpu(),    # 预测轨迹
                    select_traj=ground_truth_actions.detach().cpu(),     
                    goal_mask_select=merged_batch["goal_mask_select"],          
                    eval_type="validation",   # 文件夹名字
                    epoch=0,        
                ...
                # # 读取生成的文件并添加到 WandB
                # # 路径逻辑基于 visualize_train_new 中的定义: ./visualization/validation/epoch{epoch}/action_sampling_prediction/
                ...
                # # 遍历生成的图片添加到 wandb list
                # if os.path.exists(vis_dir):
                #     for i in range(vis_count):
                #         img_path = os.path.join(vis_dir, f"sample_0_{i}.png")
                #         if os.path.exists(img_path):
                #             wandb_images.append(wandb.Image(img_path, caption=f"Val_Ep{epoch}_Sample{i}"))

    # 计算平均 Metrics
    for k in aggregated_metrics:
        aggregated_metrics[k] /= num_batches
    ...
    if distributed_state.is_main_process:
        ...
        # 记录 Metrics
        log_dict = {f"VLA Validation/{k}": v for k, v in aggregated_metrics.items()}
        
        # 记录 Images (如果生成了)
        # if len(wandb_images) > 0:
        #     log_dict["VLA Validation/Visualization"] = wandb_images
            
        wandb_entity.log(log_dict, step=log_step)
...
def train_omnivla(cfg: OmniVLAConfig) -> None:
    ...
    recent_metrics = {
        ...
        "cost_traj": deque(maxlen=cfg.grad_accumulation_steps), # 新增                                           
    }
    ...
    for epoch in range(2):
        ...
        with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
            ...
            for batch_idx in range(cfg.max_steps):
                ...
                if distributed_state.is_main_process and log_step % cfg.wandb_log_freq == 0 and gradient_step_idx > 0: # 第一次不记录，影响观看
