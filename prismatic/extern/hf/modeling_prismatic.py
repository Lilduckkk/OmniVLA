...
class PrismaticForConditionalGeneration_MMNv1(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            # print(f"添加本体感知后 projected_patch_embeddings 形状: {projected_patch_embeddings.shape}")  # 你要添加的打印位置
            # projected_patch_embeddings = torch.cat((projected_patch_embeddings, projected_patch_embeddings, projected_patch_embeddings), dim=1)
            # print(f"拼接后 projected_patch_embeddings 形状: {projected_patch_embeddings.shape}")  # 你要添加的打印位置
            # [Diffusion] Add diffusion timestep embedding if provided
            if diffusion_timestep_embeddings is not None:
                # For simplicity, just append diffusion timestep embedding to the end of projected vision patch tokens
                projected_patch_embeddings = torch.cat(
            ...
            # print(f"添加扩散时间步长后 projected_patch_embeddings 形状: {projected_patch_embeddings.shape}")  # 你要添加的打印位置
            # Process action embeddings
            if noisy_actions is not None:
                # Get mask corresponding to all action tokens
                all_actions_mask = self._process_action_masks(labels)
...
class PrismaticForConditionalGeneration_MMNv2(PrismaticPreTrainedModel):
    def __init__(self, config: PrismaticConfig) -> None:
        super().__init__(config)
    ...
    def _build_multimodal_attention_MMN(self, input_embeddings, projected_patch_embeddings, attention_mask, attention_mask_label, modality_id):
        ...
        for ib in range(projected_patch_embeddings.shape[0]):
            ...
            # ===================== 仅新增这 3 行 =====================
            orig_obimg_len = 256  # 原始 obimg 固定长度
            total_patch_num = projected_patch_embeddings.shape[1]  # 动态获取当前特征总长度
            fused_obimg_len = total_patch_num - (256 + 1)  # 总长度 - img(256) - pose(1)（适配原始拼接顺序）
            # ========================================================

            obimg_attention_mask = torch.full((1, fused_obimg_len), fill_value=True, dtype=attention_mask.dtype, device=attention_mask.device)
            img_attention_mask = torch.full((1, 256), fill_value=modality_img, dtype=attention_mask.dtype, device=attention_mask.device)
            sate_attention_mask = torch.full((1, 512), fill_value=modality_sate, dtype=attention_mask.dtype, device=attention_mask.device)
            pose_attention_mask = torch.full((1, 1), fill_value=modality_pose, dtype=attention_mask.dtype, device=attention_mask.device)     
            projected_patch_attention_mask_b = torch.cat((obimg_attention_mask, img_attention_mask, pose_attention_mask), axis=1)
...
class OpenVLAForActionPrediction_MMNv2(PrismaticForConditionalGeneration_MMNv2):
    config_class: PretrainedConfig = OpenVLAConfig
